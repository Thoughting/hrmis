...
public final class NetWorkUtils {
	...
	private NetWorkUtils() {
		logger.warn("Oh,my god!!!How do you call this method?!");
		logger.warn("You shouldn't create me!!!");
		logger.warn("Look my doc again!!!");
	}
	
	/**
	 * 得到POST请求的长文数据（XML，JSON）
	 * 
	 * @param request
	 * @return
	 */
	 ...
	 * @param callback
	 *            请求收到响应后回调函数，参数有2个，第一个为resultCode，即响应码，比如200为成功，404为不存在，500
	 *            为服务器发生错误； 第二个为resultJson,即响应回来的数据报文
	 */
	 ...
	 *            为服务器发生错误； 第二个为resultJson,即响应回来的数据报文
	 */
	public static void doRequest(final RequestMethod method, final String url,
		...
		/*
		 * 支持不传回调函数，只输出一个警告，并改变haveCallback标识 用于一些不需要后续处理的请求，比如只是发送一个心跳包等等
		 */
		if (null == callback) {
		...
		case POST:
			...
			if (null != fileList && !fileList.isEmpty()) {
				...
				if (null != paramData) {
					builder.addPart("description", new StringBody(paramData,
				...
				if (null != paramData) {
					// 目前支持JSON格式的数据
					StringEntity jsonEntity = new StringEntity(paramData,
							ContentType.APPLICATION_JSON);
		...
		try {
			...
			logger.debug("本次请求'{}'耗时:{}ms",
					url.substring(url.lastIndexOf("/") + 1, url.length()), time);
