package com.eastcom.hrmis.modules.emp.web.controller.api;

import com.eastcom.baseframe.common.persistence.PageHelper;
import com.eastcom.baseframe.common.utils.DateUtils;
import com.eastcom.baseframe.common.utils.StringUtils;
import com.eastcom.baseframe.common.utils.excel.CustomizeToExcel;
import com.eastcom.baseframe.common.utils.excel.ExcelColumn;
import com.eastcom.baseframe.web.common.BaseController;
import com.eastcom.baseframe.web.common.model.AjaxJson;
import com.eastcom.baseframe.web.common.model.easyui.DataGridJson;
import com.eastcom.baseframe.web.common.utils.DownloadUtil;
import com.eastcom.baseframe.web.modules.sys.aop.annotation.OperationLog;
import com.eastcom.baseframe.web.modules.sys.aop.annotation.OperationType;
import com.eastcom.hrmis.modules.emp.cache.LegalHolidayCache;
import com.eastcom.hrmis.modules.emp.entity.Employee;
import com.eastcom.hrmis.modules.emp.entity.EmployeeCheckWork;
import com.eastcom.hrmis.modules.emp.entity.EmployeeCheckWorkStatMonth;
import com.eastcom.hrmis.modules.emp.service.EmployeeCheckWorkService;
import com.eastcom.hrmis.modules.emp.service.EmployeeCheckWorkStatMonthService;
import com.eastcom.hrmis.modules.emp.service.EmployeeService;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.google.common.collect.Lists;
import com.google.common.collect.Maps;
import org.apache.commons.collections.CollectionUtils;
import org.apache.commons.lang3.math.NumberUtils;
import org.apache.shiro.authz.annotation.RequiresPermissions;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RequestParam;
import org.springframework.web.bind.annotation.ResponseBody;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;
import java.util.Date;
import java.util.List;
import java.util.Map;

/**
 * 员工考勤Controller
 * @author wutingguang <br>
 */
@Controller
@RequestMapping(value="/api/emp/checkwork")
public class EmployeeCheckWorkController extends BaseController {

	@Autowired
	private EmployeeService employeeService;
	
	@Autowired
	private EmployeeCheckWorkService employeeCheckWorkService;
	
	@Autowired
	private EmployeeCheckWorkStatMonthService employeeCheckWorkStatMonthService;
	
	/**
	 * 
	 * 查询
	 * @param session
	 * @param request
	 * @param params
	 * @return
	 */
	@RequiresPermissions("emp:checkworkmgr:view")
	@SuppressWarnings("deprecation")
	@ResponseBody
	@RequestMapping(value = "/list", method = RequestMethod.POST)
	public DataGridJson list(HttpSession session, HttpServletRequest request, @RequestParam Map<String, Object> params) {
		logger.info("--获取员工考勤情况列表--");
		DataGridJson gridJson = new DataGridJson();
		try {
			List<Map<String, Object>> result = Lists.newArrayList();
			
			//查询员工档案
			int pageNo = NumberUtils.toInt((String) params.get("page"), 1);
			int pageSize = NumberUtils.toInt((String) params.get("rows"), 99999);

			String employeeDept = (String) params.get("employeeDept");
			Map<String, Object> reqParams = Maps.newHashMap();
			reqParams.put("employeeDept", employeeDept);
			reqParams.put("auditStatus", 2);
			Date d = DateUtils.parseDate((String) params.get("setWorkDate"), "yyyy年-MM月");
			d.setDate(1);
			reqParams.put("start_quitCompanyDate", DateUtils.formatDate(d)); // 如果已离职，离职时间需要大于当前月份
			d.setMonth(d.getMonth() + 1);
			d.setDate(d.getDate() - 1);
			reqParams.put("end_enrtyDate", DateUtils.formatDate(d)); // 入职时间需要小于当前月份+1
			PageHelper<Employee> empPageHelper = employeeService.find(reqParams, pageNo, pageSize);
			
			if (CollectionUtils.isNotEmpty(empPageHelper.getList())) {
				String statMonth = (String) params.get("setWorkDate");
				
				for (Employee employee : empPageHelper.getList()) {
					Map<String, Object> item = Maps.newHashMap();
					item.put("employeeId", employee.getId());
					item.put("name", employee.getName());
					item.put("code", employee.getCode());
					item.put("quitCompanyDate", employee.getQuitCompanyDate() != null ? DateUtils.formatDate(employee.getQuitCompanyDate()) : null); // 离职日期
					item.put("entryDate", employee.getEnrtyDate() != null ? DateUtils.formatDate(employee.getEnrtyDate()) : null);// 入职时间
					
					//得到员工考勤信息
					Date setWorkDate = DateUtils.parseDate(statMonth, "yyyy年-MM月");
					List<EmployeeCheckWork> checkWorks = employeeCheckWorkService.getCheckWorkMonthDate(employee.getId(), setWorkDate);
					if (CollectionUtils.isNotEmpty(checkWorks)) {
						for (EmployeeCheckWork employeeCheckWork : checkWorks) {
							item.put(DateUtils.formatDate(employeeCheckWork.getWorkDate(), "yyyy-MM-dd"), employeeCheckWork);
						}
					}

					//入职框
					if (employee.getEnrtyDate() != null){
						item.put(DateUtils.formatDate(DateUtils.addDays(employee.getEnrtyDate(),-1)),"入职");
					}

					//离职框
					if (employee.getQuitCompanyDate() != null){
						item.put(DateUtils.formatDate(DateUtils.addDays(employee.getQuitCompanyDate(),1)),"离职");
					}

					//得到员工统计信息
					EmployeeCheckWorkStatMonth employeeCheckWorkStatMonth = employeeCheckWorkStatMonthService.refreshCheckWorkStatMonth(employee,statMonth);

					item.putAll(employeeCheckWorkStatMonth.toMap());
					
					result.add(item);
				}
			}
			gridJson.setRows(result);
			gridJson.setTotal(empPageHelper.getCount());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return gridJson;
	}
	
	/**
	 * 新增修改
	 * @param session
	 * @param request
	 * @param params
	 * @return
	 */
	@RequiresPermissions("emp:checkworkmgr:edit")
	@OperationLog(content = "新增或者修改员工考勤信息" , type = OperationType.CREATE)
	@ResponseBody
	@RequestMapping(value = "/addOrUpdate", method = RequestMethod.POST)
	public AjaxJson addOrUpdate(HttpSession session, HttpServletRequest request, @RequestParam Map<String, Object> params) {
		logger.info("--新增或者修改员工考勤信息--");
		AjaxJson json = new AjaxJson();
		try {
			String employeeId = StringUtils.defaultIfBlank((String) params.get("employeeId"), "0");
			Employee employee = employeeService.get(employeeId);
			if (employee != null) {
				String id = StringUtils.defaultIfBlank((String) params.get("id"), "0");
				EmployeeCheckWork employeeCheckWork = employeeCheckWorkService.get(id);
				if (employeeCheckWork == null) {
					employeeCheckWork = new EmployeeCheckWork();
					employeeCheckWork.setEmployee(employee);
				}
				employeeCheckWork.setWorkDate(DateUtils.parseDate((String) params.get("setWorkDate")));
				employeeCheckWork.setIsAtWork(NumberUtils.toInt((String) params.get("isAtWork"), 1));
				employeeCheckWork.setAtWorkTimer(NumberUtils.toInt((String) params.get("atWorkTimer"), 0));
				employeeCheckWork.setAtWorkStatus(NumberUtils.toInt((String) params.get("atWorkStatus"), 0));
				employeeCheckWork.setAtWorkStatusMinute(NumberUtils.toInt((String) params.get("atWorkStatusMinute"), 0));
				employeeCheckWork.setAtWorkSpecialStatus(NumberUtils.toInt((String) params.get("atWorkSpecialStatus"), 0));
				employeeCheckWork.setAtHolidayType(NumberUtils.toInt((String) params.get("atHolidayType"), 0));
				employeeCheckWork.setAtHolidayHour(NumberUtils.toInt((String) params.get("atHolidayHour"), 0));
				employeeCheckWork.setAtHolidayMinute(NumberUtils.toInt((String) params.get("atHolidayMinute"), 0));
				employeeCheckWork.setAtHolidayTimer(NumberUtils.toInt((String) params.get("atHolidayTimer"), 0));
				employeeCheckWork.setOutWorkHour(NumberUtils.toInt((String) params.get("outWorkHour"), 0));
				employeeCheckWork.setOutWorkMinute(NumberUtils.toInt((String) params.get("outWorkMinute"), 0));
				employeeCheckWork.setHasOverTime(NumberUtils.toInt((String) params.get("hasOverTime"), 0));
				employeeCheckWork.setOverTimeHour(NumberUtils.toInt((String) params.get("overTimeHour"), 0));
				employeeCheckWork.setOverTimeMinute(NumberUtils.toInt((String) params.get("overTimeMinute"), 0));
				employeeCheckWork.setRemark((String) params.get("remark"));
				
				employeeCheckWorkService.saveOrUpdate(employeeCheckWork);
				
				//String statMonth = (String) params.get("setWorkDate");
				//statMonth = statMonth.split("-")[0] + "年-" + statMonth.split("-")[1] + "月";
				//employeeCheckWorkStatMonthService.refreshCheckWorkStatMonth(employee,statMonth);
			}
			json.setSuccess(true);
			json.setMessage("操作成功");
		} catch (Exception e) {
			e.printStackTrace();
			json.setSuccess(false);
			json.setMessage("操作失败");
		}
		return json;
	}
	
	/**
	 * 导出考勤Excel
	 * @param session
	 * @param request
	 * @param params
	 * @return
	 */
	@SuppressWarnings("unchecked")
	@RequiresPermissions("emp:checkworkmgr:export")
	@OperationLog(content = "导出考勤Excel" , type = OperationType.VIEW)
	@ResponseBody
	@RequestMapping(value = "/export", method = RequestMethod.POST)
	public void export(HttpSession session,HttpServletRequest request, HttpServletResponse response,
			@RequestParam Map<String, Object> params) {
		logger.info("--导出考勤Excel--");
		try {
			Date date = DateUtils.parseDate((String) params.get("setWorkDate"), "yyyy年-MM月");
			
			DataGridJson dataGridJson = list(session, request, params);
			String fileName = "员工考勤.xlsx";
			String filePath = getExportPath(request) + fileName;
			List<ExcelColumn> excelColumns = getExportExcelColumnHeader(date);
			if (CollectionUtils.isNotEmpty(excelColumns)) {
				ObjectMapper m = new ObjectMapper(); 
				String json = m.writeValueAsString(dataGridJson.getRows());
				List<Map<String,Object>> data = convertJsonArrToMap(json);
				if (CollectionUtils.isNotEmpty(data)) {
					for (Map<String, Object> map : data) {
						System.out.println("------------");
						for (String key : map.keySet()) {
							if (key.indexOf("-") != -1 && !(map.get(key) instanceof String)) {
								Map<String,Object> temp = (Map<String, Object>) map.get(key);
								if (temp != null) {
									List<String> gridCellDescribe = (List<String>) temp.get("gridCellDescribe");
									String describe = gridCellDescribe.get(1);
									describe = describe.replace("<font size=\"4\">", "");
									describe = describe.replace("</font>", "");
									describe = describe.replace("&nbsp;", "");
									describe = describe.replace("<br/>", "\n");
									map.put(key, describe);
								} else {
									map.put(key, 0);
								}
							}
							if ("kgTj".equals(key) || "cdTj".equals(key) || "ztTj".equals(key)) {
								map.put(key, ((String )map.get(key)).replace("<br/>", "\n"));
							}
						}
					}
				}
				CustomizeToExcel.toFile(excelColumns ,data ,filePath);
			}
			DownloadUtil.downloadFile(filePath, fileName, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	
	/**
	 * 判断考勤日期是否可编辑
	 * @param session
	 * @param request
	 * @param params
	 * @return
	 */
	@OperationLog(content = "" , type = OperationType.CREATE)
	@ResponseBody
	@RequestMapping(value = "/checkEdit", method = RequestMethod.POST)
	public AjaxJson checkEdit(HttpSession session, HttpServletRequest request, @RequestParam Map<String, Object> params) {
		logger.info("--判断考勤日期是否可编辑--");
		AjaxJson json = new AjaxJson();
		try {
			Date date = DateUtils.parseDate((String) params.get("date"), "yyyy-MM-dd");
			if (date != null && getEditDayCount(date) <= 3) {
				json.setSuccess(true);
			}
		} catch (Exception e) {
			e.printStackTrace();
			json.setSuccess(false);
			json.setMessage("操作失败");
		}
		return json;
	}
	
	/**
	 * 3个工作日，除去周六日和设置的节日
	 * 
	 * @return
	 */
	@SuppressWarnings({"deprecation"})
	private int getEditDayCount(Date date){
		int count = 0;
		if (!("星期六".equals(DateUtils.getWeekOfDate(date)) || "星期日".equals(DateUtils.getWeekOfDate(date)) || LegalHolidayCache.isLegalHoliday(date))) {
			count++;
		}
		date.setDate(date.getDate() + 1);
		if (date.getDate() <= new Date().getDate() && DateUtils.getDistanceOfTwoDate(date, new Date()) >= 0) {
			count += getEditDayCount(date);
		}
		return count;
	}
	
	/**
	 * 根据月份得到表格头
	 * @param session
	 * @param request
	 * @param params
	 * @return
	 */
	@OperationLog(content = "" , type = OperationType.CREATE)
	@ResponseBody
	@RequestMapping(value = "/column", method = RequestMethod.POST)
	public AjaxJson column(HttpSession session, HttpServletRequest request, @RequestParam Map<String, Object> params) {
		logger.info("--根据月份得到表格头--");
		AjaxJson json = new AjaxJson();
		try {
			Date date = DateUtils.parseDate((String) params.get("date"), "yyyy年-MM月");
			List<List<Map<String, Object>>> columns = getMonthColumnHeader(date);
			json.setModel(columns);
			json.setSuccess(true);
			json.setMessage("操作成功");
		} catch (Exception e) {
			e.printStackTrace();
			json.setSuccess(false);
			json.setMessage("操作失败");
		}
		return json;
	}
	
	/**
	 * 月份表头
	 * 
	 * @return
	 */
	@SuppressWarnings("deprecation")
	private List<List<Map<String, Object>>> getMonthColumnHeader(Date date){
		List<List<Map<String, Object>>> columns = Lists.newArrayList();
		List<Map<String, Object>> column_1 = Lists.newArrayList();
		List<Map<String, Object>> column_2 = Lists.newArrayList();
		Map<String, Object> item = Maps.newHashMap();
		
		//日期头
		int maxDay = DateUtils.getMonthMaxDay(date);
		for (int i = 1; i <= maxDay; i++) {
			date.setDate(i);
			Map<String, Object> item_1 = Maps.newHashMap();
			item_1.put("title", DateUtils.formatDate(date, "dd"));
			item_1.put("align", "center");
			item_1.put("width", 50);
			item_1.put("colspan", 1);
			column_1.add(item_1);
			
			Map<String, Object> item_2 = Maps.newHashMap();
			item_2.put("field", DateUtils.formatDate(date, "yyyy-MM-dd"));
			item_2.put("title", DateUtils.getWeekOfDate(date).replaceAll("星期", ""));
			item_2.put("align", "center");
			item_2.put("width", 50);
			column_2.add(item_2);
		}
		
		//统计头
		
		//出勤及加班加点
		item = Maps.newHashMap();
		item.put("title", "出勤及加班加点");
		item.put("align", "center");
		item.put("width", 100);
		item.put("colspan", 8);
		column_1.add(item);
		
		Map<String, Object> item_2 = Maps.newHashMap();
		item_2.put("field", "cqscDay");
		item_2.put("title", "出勤天数");
		item_2.put("align", "center");
		item_2.put("width", 100);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "jjrJbscDay");
		item_2.put("title", "节日");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "xxrJbscDayDesc");
		item_2.put("title", "休日");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "jblXscDay");
		item_2.put("title", "加点");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "stypeCqscDay");
		item_2.put("title", "S");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "vtypeCqscDay");
		item_2.put("title", "V");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "gtypeCqscDay");
		item_2.put("title", "G");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "ztypeCqscDay");
		item_2.put("title", "Z");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		//缺勤情况
		item = Maps.newHashMap();
		item.put("title", "缺勤情况");
		item.put("align", "center");
		item.put("width", 100);
		item.put("colspan", 6);
		column_1.add(item);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "zcxjScDay");
		item_2.put("title", "正休");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "bxjScDay");
		item_2.put("title", "补休");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "nxjScDay");
		item_2.put("title", "年休");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "xinjiaScDay");
		item_2.put("title", "薪假");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "bjScDay");
		item_2.put("title", "病假");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "shjScDay");
		item_2.put("title", "事假");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);

		item_2 = Maps.newHashMap();
		item_2.put("field", "gsjScDay");
		item_2.put("title", "工伤假");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		//违纪
		item = Maps.newHashMap();
		item.put("title", "违纪");
		item.put("align", "center");
		item.put("width", 100);
		item.put("colspan", 3);
		column_1.add(item);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "kgTj");
		item_2.put("title", "旷工");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "cdTj");
		item_2.put("title", "迟到");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		item_2 = Maps.newHashMap();
		item_2.put("field", "ztTj");
		item_2.put("title", "早退");
		item_2.put("align", "center");
		item_2.put("width", 50);
		column_2.add(item_2);
		
		columns.add(column_1);
		columns.add(column_2);
		
		return columns;
	}
	
	/**
	 * excel导出月份表头
	 * @param date
	 * @return
	 */
	@SuppressWarnings("deprecation")
	private List<ExcelColumn> getExportExcelColumnHeader(Date date){
		List<ExcelColumn> excelColumns = Lists.newArrayList();
		
		excelColumns.add(new ExcelColumn("编号", "code", 80));
		excelColumns.add(new ExcelColumn("姓名", "name", 80));
		
		//日期头
		int maxDay = DateUtils.getMonthMaxDay(date);
		for (int i = 1; i <= maxDay; i++) {
			date.setDate(i);
			ExcelColumn dateColumn = new ExcelColumn(DateUtils.formatDate(date, "dd"), "", 50);
			dateColumn.getChildren().add(new ExcelColumn(DateUtils.getWeekOfDate(date).replaceAll("星期", ""), DateUtils.formatDate(date, "yyyy-MM-dd"), 50));
			excelColumns.add(dateColumn);
		}
		
		//统计头
		
		//出勤及加班加点
		ExcelColumn statColumn = new ExcelColumn("出勤及加班加点", "", 100);
		statColumn.getChildren().add(new ExcelColumn("出勤天数", "cqscDay", 100));
		statColumn.getChildren().add(new ExcelColumn("节日", "jjrJbscDay", 50));
		statColumn.getChildren().add(new ExcelColumn("休日", "xxrJbscDayDesc", 50));
		statColumn.getChildren().add(new ExcelColumn("加点", "jblXscDay", 50));
		statColumn.getChildren().add(new ExcelColumn("S", "stypeCqscDay", 50));
		statColumn.getChildren().add(new ExcelColumn("V", "vtypeCqscDay", 50));
		statColumn.getChildren().add(new ExcelColumn("G", "gtypeCqscDay", 50));
		statColumn.getChildren().add(new ExcelColumn("Z", "ztypeCqscDay", 50));
		excelColumns.add(statColumn);
		
		//缺勤情况
		statColumn = new ExcelColumn("缺勤情况", "", 100);
		statColumn.getChildren().add(new ExcelColumn("正休", "zcxjScDay", 50));
		statColumn.getChildren().add(new ExcelColumn("补休", "bxjScDay", 50));
		statColumn.getChildren().add(new ExcelColumn("年休", "nxjScDay", 50));
		statColumn.getChildren().add(new ExcelColumn("薪假", "xinjiaScDay", 50));
		statColumn.getChildren().add(new ExcelColumn("病假", "bjScDay", 50));
		statColumn.getChildren().add(new ExcelColumn("事假", "shjScDay", 50));
		statColumn.getChildren().add(new ExcelColumn("工伤假", "gsjScDay", 50));
		excelColumns.add(statColumn);
		
		//违纪
		statColumn = new ExcelColumn("违纪", "", 100);
		statColumn.getChildren().add(new ExcelColumn("旷工", "kgTj", 50));
		statColumn.getChildren().add(new ExcelColumn("迟到", "cdTj", 50));
		statColumn.getChildren().add(new ExcelColumn("早退", "ztTj", 50));
		excelColumns.add(statColumn);
		
		return excelColumns;
	}
}
