...
Constant = {
	init : function(){
		Constant.$grid_list_url = basePath + '/api/emp/dept/list';
		Constant.$addEdit_form_url = basePath + "/api/emp/dept/addOrUpdate";
		Constant.$delete_url = basePath + "/api/emp/dept/delete";
	}
}
	...
	init : function(){
		...
		Component.$addEditForm.form({
			...
			onSubmit : function(){
				...
				//判断工作时长只能为数值
				if (!$.isNumeric($("#ud_workTimer").val())) {
					isValid = false;
					$.messager.alert('提示','工作时长须为数值!');
				}
				if (!isValid){
		...
		$("#editBtn").linkbutton({
			onClick : function(){
				...
				if (selectNode != null) {
					...
					$("#ud_workTimer").textbox("setValue",selectNode.workTimer);
