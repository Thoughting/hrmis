...
Constant = {
	init : function(){
		Constant.$grid_list_url = basePath + '/api/emp/wageplan/list';
		Constant.$addEdit_form_url = basePath + "/api/emp/wageplan/addOrUpdate";
		Constant.$delete_url = basePath + "/api/emp/wageplan/delete";
		
		Constant.$setWageCountItemTreeList = basePath + "/api/emp/wageplan/wageCountItemTreeList";
		Constant.$setPostTreeList = basePath + "/api/emp/wageplan/postTreeList";
		Constant.$updateSet_url = basePath + "/api/emp/wageplan/updateSet";
	}
}
...
Component = {
	init : function(){
		...
		Component.$setWageCountItemTree = $("#setWageCountItemTree");
	...
	init : function(){
		...
		$("#addSettingBtn").linkbutton({
			onClick : function(){
				var selectNode = Component.$grid.datagrid('getSelected');
				if (selectNode != null) {
					Component.$setWageCountItemTree.tree({
						url : Constant.$setWageCountItemTreeList,
		...
		$("#confirmSetBtn").linkbutton({
			onClick : function(){
				var checkWageCountItemNodes = Component.$setWageCountItemTree.tree('getChecked');
				var checkPostNodes = Component.$setPostTree.tree('getChecked');
				var selectNode = Component.$grid.datagrid('getSelected');
				if (checkWageCountItemNodes != null && checkPostNodes != null && selectNode != null) {
					...
					$.ajax({
						...
						dataType : 'json',
						data : {
							checkWageCountItemNodes : JSON.stringify(checkWageCountItemNodes),
